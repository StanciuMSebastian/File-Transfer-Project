...
#include"AllUsers.h"
#include"User.h"
void UpdateQuery::showSQLError(unsigned int handleType, const SQLHANDLE& handle)
{
...
void UpdateQuery::ExecuteQuery()
{
	...
	string SQLQuerry;
	int i = getUpdateId();
	SQLQuerry = "UPDATE Clienti SET Password='" + getParola() + "' " + "WHERE Username=" + "'" + AllUsers::ListOfUsers[i]->getUsername() + "'";
	char SQLQuery[2048];
	...
	if (SQL_SUCCESS != SQLExecDirect(SQLStatementHandle, (SQLCHAR*)SQLQuery, SQL_NTS)) {
		...
		while (SQLFetch(SQLStatementHandle) == SQL_SUCCESS) {
			...
			SQLGetData(SQLStatementHandle, 2, SQL_C_DEFAULT, &passwd, sizeof(passwd), NULL);
			SQLGetData(SQLStatementHandle, 3, SQL_C_DEFAULT, &rol, sizeof(rol), NULL);
			/*users[i]->setUsername(username);
			v[i]->setPassword(passwd);
			v[i]->setRank(rol);*/

			//users.setAll(username, passwd, rol);
