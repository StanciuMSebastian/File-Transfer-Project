...
void SelectQuery::showSQLError(unsigned int handleType, const SQLHANDLE& handle)
{
...
void SelectQuery::ExecuteQuery()
{
		...
		if (SQL_SUCCESS != SQLExecDirect(SQLStatementHandle, (SQLCHAR*)SQLQuery, SQL_NTS)) {
			...
			while (SQLFetch(SQLStatementHandle) == SQL_SUCCESS) {
				...
				SQLGetData(SQLStatementHandle, 4, SQL_C_DEFAULT, &ID, sizeof(int), NULL);
				/*users[i]->setUsername(username);
				v[i]->setPassword(passwd);
				v[i]->setRank(rol);*/
				
				users.setAll(username, passwd, rol, ID);
